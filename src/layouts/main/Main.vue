        ...
        <div class="router-view">
          <div class="router-content">
            ...
            <div class="content-area__content">
              ...
              <back-to-top bottom="90px" :right="$vs.rtl ? 'calc(100% - 2.2rem - 38px)' : '20px'" visibleoffset="500" v-if="!hideScrollToTop">
                <vs-button icon-pack="feather" icon="icon-arrow-up" class="shadow-lg btn-back-to-top" />
              </back-to-top>

              <transition :name="routerTransition" mode="out-in">
                <router-view :mono="mono" @changeRouteTitle="changeRouteTitle" @setAppClasses="(classesStr) => $emit('setAppClasses', classesStr)" />
              </transition>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getIDB(){
      return this.$store.dispatch('getPinjaman')
  ...
  created() {
    ...
    let datosDB = JSON.parse(localStorage.getItem('cobaindah-v2'))
    if (datosDB === null) {
      this.mono = [];
    }else{
      this.mono = datosDB;
      localStorage.setItem('cobaindah-v2', JSON.stringify(this.mono));
    }
  }
}
